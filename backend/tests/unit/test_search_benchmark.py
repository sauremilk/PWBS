...
FIXTURE_PATH = Path(__file__).parent.parent / "fixtures" / "search_relevance" / "query_document_pairs.json"
...
class TestSearchRelevanceBenchmark:
    ...
    def reranker(self) -> SearchReranker:
        return SearchReranker(config=RerankerConfig(
            cosine_weight=0.6,
            rrf_weight=0.3,
    ...
    def test_precision_at_10(
        ...
        for pair in benchmark_pairs:
            ...
            hits = sum(
                1 for r in reranked if r.title in relevant_titles
            )
        ...
        assert avg_precision >= 0.6, (
            f"Average Precision@10 = {avg_precision:.3f}, expected >= 0.6"
        )
