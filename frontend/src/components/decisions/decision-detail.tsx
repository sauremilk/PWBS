...
const STATUS_OPTIONS: { value: DecisionStatus; label: string; icon: typeof Clock }[] = [
...
function ArgumentList({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4">
      ...
      {items.length === 0 ? (
        <p className="text-xs text-gray-400 italic">Keine Eintr&auml;ge</p>
      ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-gray-500">
            ...
            <span>
              Erstellt: {new Date(decision.created_at).toLocaleDateString("de-DE")}
            </span>
          </div>
        </div>
            ...
            <div className="flex items-center gap-1">
              {STATUS_OPTIONS.map((opt) => {
                ...
                return (
                  ...
                  >
                    <OptIcon className="mr-1 inline h-3 w-3" />
                    {opt.label}
                  </button>
                );
              })}
      ...
      <div className="text-xs text-gray-400">
        Zuletzt aktualisiert: {new Date(decision.updated_at).toLocaleString("de-DE")}
      </div>
    </div>
  );
}
